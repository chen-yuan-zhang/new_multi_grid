...
Given (1) the current observation as an image, (2) a goal as a white dot, and
(3) a behavior type, you must output
ONLY the next 3 actions as a canonical token from the environment's action space.
...
- Valid action set: {right, down, left, up, stop}, stop is a speical token that means the episode ends.
"""
if_shuffle = False
...
Format:
Return THREE action token from the allowed set. No extra words.
"""

dataset_path = "/home/sukai/Project/chenyuan_project/new_multi_grid/data/training_data/actor_moves_dataset_fuse_image"
...
if __name__ == "__main__":
    project_name = "qwen2.5_vlm_prediction_fuse_image" + "no_shuffle" if not if_shuffle else "shuffle"
    ...
    model = FastVisionModel.get_peft_model(
        ...
        lora_dropout = 0, # Added dropout for regularization (10%)
    ...
    trainer = SFTTrainer(
        ...
        args = SFTConfig(
            ...
            warmup_steps = 50,                  # Increased warmup to prevent early overfitting
            num_train_epochs = 4,              # Increased max epochs, early stopping will handle it
            learning_rate = 1e-4,               # Reduced LR to prevent overfitting (was 1.5e-4)
            logging_steps = 50,                 # More frequent logging
            optim = "adamw_8bit",
            weight_decay = 0.01,
            lr_scheduler_type = "cosine",
            seed = 3407,
            output_dir = "data/trained_models/neuro_predictor_fuse_image",
