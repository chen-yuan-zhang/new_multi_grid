...
def astar_wall(pos_state, target, env, cost = None, heuristic=heuristic_manhattan, max_iter=None):
    ...
    while pq:
        ...
        successors = get_successor_withwall(env, current.pos_state)
...
def execute_action_withwall(pos_state, action, env):
    """
    Execute an action in the environment.
...
def get_successor_withwall(env, pos_state):
    """
    Generate the next position and direction given the current position and direction.
    ...
    for action in actions:
        status, successor = execute_action_withwall((pos, dir), action, env)
        if status:
            successors.append((action, successor))
