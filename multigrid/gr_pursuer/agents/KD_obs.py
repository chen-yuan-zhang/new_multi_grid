...
import math
import numpy as np
from multigrid.core.constants import DIR_TO_VEC, Direction,OBJECT_TO_IDX
...
class BeliefUpdateObserver(BaseAgent):
    def __init__(self, env, init_actor_belief = None, init_goal_belief = None):
        ...
        self.all_goals = [g for g in env.goals]
        ...
        for goal in self.goals:
            ...
            for ev in plan_events:
                ...
                stype = ev['type']
                color = ev.get('color', None)
                chain.append((stype, pos, color))   # 用tuple记录subgoal
                self.all_goals.append(pos)
        ...
        print("subgoal_to_goals:",self.subgoal_to_goals)

    # === [新增] 构建 subgoal→goals 映射 ===
    ...
    def update_goal_belief_from_subgoal(self, subgoal_posterior: dict, renorm=True):
        new_goal_belief = {g: 0.0 for g in self.goals}
    ...
    def compute_action(self, obs):
        ...
        for v in visible:
            ...
            if base_type == OBJECT_TO_IDX['key'] and cur_type != OBJECT_TO_IDX['key']:
                completed.append(('pickup', (x, y), v.get('color', None)))
    
            # 门：如果当前位置是门，且可从可见信息判断为open => 门被打开
            # 注：有些实现里 base_grid 只有type没有state；只要FOV里能读到 'state'=='open' 就触发
            if base_type == OBJECT_TO_IDX['door']:
                v_state = v.get('state', None)
                if v_state == 'open':
                    completed.append(('open', (x, y), v.get('color', None)))

        # === [NEW] 将事件转换为 subgoal 完成，推进任务链 + 概率继承 ===
        for s in completed:
            self.on_subgoal_completed(s)

        # === [NEW] 由“当前需要的subgoal”反推 goal 概率（你的等分规则）
        # 构造 subgoal_posterior：把当前每个goal的概率“投影”到它当前需要的subgoal上
        subgoal_posterior = {}
        ...
        for g in self.goals:
            ...
            subgoal_posterior[s] = subgoal_posterior.get(s, 0.0) + self.goal_belief[g]
        print(subgoal_posterior)
    ...
    #     """
    #     Simpler version: use Manhattan distance from every free (x,y,dir) state to each goal.
    #     - Ignores obstacles; direction does not affect the distance.
    #     - Returns a dict keyed by ((pos_xy, dir), goal_xy) -> distance (float),
    #       matching your original 'adjusted_dist_matrix' interface.
    #     """
    #     import numpy as np
    
    #     H, W = self.env.width, self.env.height
    #     num_dirs = 4
    
    #     # Determine which cells to include. If you have OBJECT_TO_IDX, prefer that.
    #     WALL_IDX = 2  # <- if you have OBJECT_TO_IDX['wall'], use it instead of hardcoding 2
    #     # Use all non-wall cells as "free"
    #     free_cells = np.argwhere(self.env.base_grid != WALL_IDX)
    
    #     # goals source: keep compatibility with your original attribute
    #     goals_list = self.all_goals
    
    #     adjusted_dist_matrix = {}
    
    #     # Compute Manhattan distance for each free cell, each direction, each goal
    #     # Note: direction does not change the distance; we still index per-dir to match your state space.
    #     print("start")
    ...
    #                 manhattan = abs(x - gx) + abs(y - gy)  # L1 distance
    #                 adjusted_dist_matrix[(((x, y), d), (gx, gy))] = float(manhattan)
