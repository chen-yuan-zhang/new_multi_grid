...
class AGREnv(MultiGridEnv):
    ...
    * Timeout (see ``max_steps``)
    ...
    def save_config(self, filename="run1_config.json", extra=None):
        config = {
            "grid_size": int(self.grid_size),
            "agents_start_pos": np.array(self.agents_start_pos).tolist(),
            "agents_start_dir": np.array(self.agents_start_dir).tolist(),
            "base_grid": np.array(self.base_grid).tolist(),
            "num_goals": int(self.num_goals),
            "goal": [int(x) for x in self.goal],
            "goals" : np.array(self.goals).tolist(),
            "initial_distance": float(self.initial_distance),
            "enable_hidden_cost": bool(self.enable_hidden_cost),
        }
    ...
    def load_config(self, filename="run1_config.json"):
        with open(os.path.join("outputs", filename), "r") as f:
            config = json.load(f)
    
        self.grid_size = config["grid_size"]
        self.agents_start_pos = config["agents_start_pos"]
        self.agents_start_dir = config["agents_start_dir"]
        self.base_grid = np.array(config["base_grid"])
        self.goals = [tuple(g) for g in config["goals"]]
        self.goal = tuple(np.int64(x) for x in config["goal"])
        self.num_goals = config["num_goals"]
        self.initial_distance = config["initial_distance"]
        self.enable_hidden_cost = config["enable_hidden_cost"]
        self.TargetAgent_actions = config.get("TargetAgent_actions", [])
