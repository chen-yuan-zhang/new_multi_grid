...
def run_scenario(scenario_config: Dict[str, Any], verbose: bool = False) -> Tuple[bool, int, Dict[str, Any]]:
    ...
    observer_agent = BeliefUpdateObserver(env, use_neural_predictor=True, use_neural_when_in_view_only=False)
...
def main(dataset_path: Optional[str] = None, verbose: bool = False) -> None:
    ...
    for i, (idx, scenario_row) in enumerate(scenarios_df.iterrows()):
        ...
        if scenario_num % 50 == 0 or scenario_num in [100, 200, 300, 400, 500, 600]:
            temp_filename = f"ns_symbolic_evaluation_results_temp_{scenario_num}_{dataset_path.replace('.csv', '').replace('/', '_')}.csv"
    ...
    final_output_file = f"ns_symbolic_evaluation_results_{int(time())}.csv"
